<?php
require 'config.php'; // Make sure you have a valid database connection in this file
require 'fpdf.php';
...
if (isset($_POST["import"])) {
    ...
    foreach ($reader as $row) {
        ...
        $stmt = $con->prepare("INSERT INTO completion_letter (sname, cnumber, uemail, domain) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $sname, $cnumber, $uemail, $udomain);
        $stmt->execute();
        $stmt->close();
    }
...
$query = "SELECT * FROM completion_letter WHERE uemail LIKE '%$searchKeyword%'";
    ...
    <div class="search-container">
        ...
        <table id="student">
            <tr id="th">
                ...
                <td>Email</td>
                <td>Domain</td>
                <td>Completion Letter</td>
                <td>Completion Letter as Pdf</td>
                <td>Send Email</td>
            </tr>
            <?php
            ...
            if (!$result || mysqli_num_rows($result) === 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    imagettftext($image, 45, 0, 350, 750, $color, $font, $name);
                    // imagettftext($image, 20, 0, 180, 568, $color, $font, $email);
                    imagettftext($image, 45, 0, 350, 950, $color, $font, $domain);
                    imagettftext($image, 28, 0, 445, 1218, $color, $font, $date);
                    imagettftext($image, 25, 0, 1762, 1220, $color, $font, $cno);
                    imagejpeg($image, "Certificates/" . $name . ".jpg");

                    // PDF generation
                    $pdf = new FPDF('P', 'in', [20.7, 20.27]);
                    $pdf->AddPage();
